...
import com.google.androidcalculator.databinding.ActivityMainBinding
...
import com.google.androidcalculator.scriptcodeUI.AllCodesActivity
...
class MainActivity : AppCompatActivity() {
    ...
    val SHEL_EXECUTE_ERROR = "SHEL_EXECUTE_ERROR"
  ...
  */
    ...
    fun feeback(menuItem: MenuItem) {
//        Themes.openDialogThemeSelector(this)
        Themes.basicAlert(this)
    }
    ...
    private fun updateDisplay(view: View, value: String) {
        ...
        lifecycleScope.launch(Dispatchers.Default) {
            ...
            withContext(Dispatchers.Main) {
                ...
                checkMasterValue(newValueFormatted)
                    ...
                    if ((result * 10) % 10 == 0.0) {
                        ...
                        withContext(Dispatchers.Main) {
                            if (formattedResult != calculation) {
                                ...
                                checkMasterValue(formattedResult)
    ...
    private fun checkMasterValue(formattedResult: String) {
        try {
            val masterCode = myPrefsManager.read().toString()
            val resultCode = formattedResult.replace(",", "")
            /* val formatter = DecimalFormat("#,###,###")
             val formattedMastercode: String = formatter.format(mastercode)
            */

            if (resultCode == masterCode)
                showDialog()
            else {
                getAllScripts(resultCode)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private fun runScript(scriptCodes: ScriptCodes) {
        try {
            /*          val stringBuilder = StringBuilder()
                      try {
                          val process: Process = Runtime.getRuntime().exec(arrayOf("/system/xbin/su", "-c", "reboot now"))
            ...
            var line = ""
            val command = scriptCodes.script
    ...
    private fun showDialog() {
        if (dialog == null) {
            ...
            buttonViewAll.setOnClickListener {
                startActivity(Intent(this, AllCodesActivity::class.java))
                dialog!!.dismiss()
            }
            buttonSave.setOnClickListener {
                val scriptcode = editTextScriptCode.text.toString()
                if (editTextScript.text!!.isNotEmpty() && editTextScriptCode.text!!.isNotEmpty()) {
                    if (scriptcode != myPrefsManager.read().toString()) {
                        saveScriptandCode(
    ...
    private fun getAllScripts(resultCode: String) {
        var allCodeList = mutableListOf<ScriptCodes>()
        lifecycleScope.launch {
            scriptDatabase.getScripts().collect { notesList ->
                if (notesList.isNotEmpty()) {
                    val filterList = notesList.filter { it.code.toString() == resultCode }
                    if (!filterList.isNullOrEmpty())
                        runScript(filterList[0])
                } else {
                    //Nothing
                }
            }
